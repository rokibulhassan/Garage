// https://raw.github.com/kball/singular_sortable/patch-1/lib/jquery.singularsortable.js
(function($) {
	$.widget("ui.singular_sortable", {
		...
		_onSortUpdate : function(e, ui) {
			...
			var newPosition = me._reorderPositions(ui.item);
			me._trigger("update", e, $.extend(ui, {'numericalPosition' : newPosition}));
		},
		
		_reorderPositions : function(sortableItem, newPosition) {
      // Where are we now?
      ...
      this._setListingPosition(sortableItem, -9999);
      this._shiftLowerUp(oldPosition);
    ...
    _findNewListingPosition : function(sortableItem) {
      ...
      // NB! Rendering in REVERSE order of positionAttribute
      // It assumes (position + 1) of the element right beneath it in the list
      if(next && (next.attr(this.options.positionAttribute) != undefined)) {
        position = this._findListingPosition(next) + 1
      } else {
        // Or, if nothing beneath it, then it assumes the position of the item right above it
        var prev = sortableItem.prev();
        if(prev && (prev.attr(this.options.positionAttribute) != undefined)) {
          position = this._findListingPosition(prev)
        }
      }
    ...
    _shiftLowerUp : function(position) {
      this._shiftLowerBy(position, -1);
    },
  
    _shiftLowerDown : function(position) {
      this._shiftLowerBy(position, 1);
    },
  
    _shiftLowerBy : function(position, byAmount) {
			var me = this;
      jQuery('[' + this.options.positionAttribute + ']', this.element).each(function(i) {
